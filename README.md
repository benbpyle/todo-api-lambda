...
```bash
todo-lambda-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
todo-lambda-api$ sam local start-api
todo-lambda-api$ curl http://localhost:3000/
```
...
```bash
todo-lambda-api$ sam logs -n HelloWorldFunction --stack-name todo-lambda-api --tail
```
